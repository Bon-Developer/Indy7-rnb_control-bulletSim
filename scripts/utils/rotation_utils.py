...
if hasattr(Rotation, "as_dcm"):
    Rotation_as_dcm = Rotation.as_dcm
else:
    Rotation_as_dcm = Rotation.as_matrix

if hasattr(Rotation, "from_dcm"):
    Rotation_from_dcm = Rotation.from_dcm
else:
    Rotation_from_dcm = Rotation.from_matrix
...
def T2xyzrvec(T):
    return T[:3,3].tolist(), Rotation_from_dcm(T[:3,:3]).as_rotvec().tolist()

##
# @return tuple(xyz, quaternion)
def T2xyzquat(T):
    return T[:3,3].tolist(), Rotation_from_dcm(T[:3,:3]).as_quat().tolist()
...
def T_xyzquat(xyzquat):
    return SE3(Rotation_as_dcm(Rotation.from_quat(xyzquat[1])), xyzquat[0])
